...
```
YOU HAVE A GOOD HANDLE ON INDEXING. CAN YOU FIGURE OUT A WAY TO AUTOMATE THIS CODE SO THAT IT DOESN'T REQUIRE A HUMAN TO READ THE VALUES AND DETERMINE THE ROW NUMBERS FOR THE INDICES?  HERE IS A POSSIBILITY:

```{r}
#create new column
Soil_Data$Forest <-"" #creates new empty character column
#sort forests and fill in column
Degrasse<-"DEGRASSE*" #creates wildcard to search
fix <- grep(Degrasse, Soil_Data$Field.Name)
Soil_Data$Forest[fix]<- "Degrasse"

Don <-"DONNERVILLE*"
fix <- grep(Don, Soil_Data$Field.Name)
Soil_Data$Forest[fix]<- "Donnerville"

Kip <-"KIP*"
fix <-grep(Kip, Soil_Data$Field.Name)
Soil_Data$Forest[fix] <- "Kip Tract"

Pea <-"PEAVINE*"
fix <-grep(Pea, Soil_Data$Field.Name)
Soil_Data$Forest[fix] <- "Peavine"

SH <-"SOUTH HAMMOND*"
fix <-grep(SH, Soil_Data$Field.Name)
Soil_Data$Forest[fix] <- "South Hammond"
